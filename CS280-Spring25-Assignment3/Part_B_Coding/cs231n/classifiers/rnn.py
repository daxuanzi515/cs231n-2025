...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (3) RNN or LSTM forward pass
        if self.cell_type == "rnn":
            h, rnn_cache = rnn_forward(embedded, h0, Wx, Wh, b)
        elif self.cell_type == "lstm":
            h, lstm_cache = lstm_forward(embedded, h0, Wx, Wh, b)
        ...
        # (2) Backward through word embedding layer
        dW_embed = word_embedding_backward(dx, embed_cache)
        grads["W_embed"] = dW_embed

        # (1) Backward through affine layer
    ...
    def sample(self, features, max_length=30):
        ...
        # For LSTM, also initialize the cell state
        if self.cell_type == "lstm":
            c = np.zeros_like(h)

        # Initialize the first word as <START> token
